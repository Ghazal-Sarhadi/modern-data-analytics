...
OUTPUT_DIR = TRANSFORMED_DATA_DIR / "combined_station_groups_1km" / "outputs"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

SITES_CSV = BASE_DIR / "data" / "sites.csv"
MONTHLY_1KM_CSV = TRANSFORMED_DATA_DIR / "model_outputs" / "station_month_bike_crash_dataset_1km.csv"
MATCHED_1KM_CSV = TRANSFORMED_DATA_DIR / "crash_outputs" / "crashes_bike_only_matched_to_sites_1km.csv"

GROUP_MAPPING_CSV = OUTPUT_DIR / "group_mapping.csv"
GROUPED_MONTHLY_CSV = OUTPUT_DIR / "station_group_month_bike_crash_dataset_1km.csv"
...
def build_grouped_monthly(mapping_df: pd.DataFrame) -> pd.DataFrame:
    monthly_df = pd.read_csv(MONTHLY_1KM_CSV)
    ...
    for (group_id, year, month), part in monthly_df.groupby(["group_id", "year", "month"], sort=True):
        ...
        row["station_to_weather_grid_km"] = weighted_mean(
            part["station_to_weather_grid_km"],
            part["weather_hours_available"],
        )
    ...
    matched_df = pd.read_csv(MATCHED_1KM_CSV)
    ...
    crash_monthly_df = (
        ...
        .rename(
            columns={
                ...
                "crash_row_id": "bike_crash_count_1km_grouped",
            }
        )
    )
    crash_monthly_df["bike_crash_happened_1km_grouped"] = (
        crash_monthly_df["bike_crash_count_1km_grouped"] > 0
    ).astype(int)
    ...
    dataset_df["bike_crash_count_1km_grouped"] = dataset_df["bike_crash_count_1km_grouped"].fillna(0)
    dataset_df["bike_crash_happened_1km_grouped"] = (
        dataset_df["bike_crash_happened_1km_grouped"].fillna(0).astype(int)
    )
...
def build_next_month_dataset(grouped_monthly_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["bike_crash_happened_1km_grouped_lag1"] = df.groupby("group_id")["bike_crash_happened_1km_grouped"].shift(1)
    df["bike_crash_happened_1km_grouped_lag2"] = df.groupby("group_id")["bike_crash_happened_1km_grouped"].shift(2)
    df["bike_crash_count_1km_grouped_roll3_sum"] = (
        df.groupby("group_id")["bike_crash_count_1km_grouped"]
        .transform(lambda s: s.shift(1).rolling(window=3, min_periods=1).sum())
    )
    df["bike_crash_count_1km_grouped_roll6_sum"] = (
        df.groupby("group_id")["bike_crash_count_1km_grouped"]
    ...
    df["bike_crashes_per_1000_cyclists_lag1"] = (
        1000 * df["bike_crash_count_1km_grouped_lag1"] / df["total_count_sum_lag1"].replace(0, pd.NA)
    )
    ...
    df["target_bike_crash_count_next_month_1km_grouped"] = df.groupby("group_id")["bike_crash_count_1km_grouped"].shift(-1)
    df["target_bike_crash_happened_next_month_1km_grouped"] = (
        df.groupby("group_id")["bike_crash_happened_1km_grouped"].shift(-1)
    )
...
def write_summary(mapping_df: pd.DataFrame, grouped_monthly_df: pd.DataFrame, grouped_next_df: pd.DataFrame) -> None:
    ...
    positive_share = float(grouped_next_df["target_bike_crash_happened_next_month_1km_grouped"].mean())
    mean_count = float(grouped_monthly_df["bike_crash_count_1km_grouped"].mean())

    summary = [
        "Combined station group experiment",
