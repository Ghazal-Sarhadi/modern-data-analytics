...
sites_df = sites_df[~sites_df["site_id"].isin(PROBLEMATIC_SENSORS)]
...
# %%
# Keep only crashes where one of the two road users is a bicycle and that happened in Flanders 
bike_crashes_df = crashes_df[
    (
    (crashes_df["CD_ROAD_USR_TYPE1"] == bicycle_code)
    | (crashes_df["CD_ROAD_USR_TYPE2"] == bicycle_code)
    ) &
    (crashes_df["TX_RGN_COLLISION_NL"] == "Vlaams Gewest")
].copy()

#Drop null cordinates
bike_crashes_df = bike_crashes_df.dropna(subset=["MS_X_COORD","MS_Y_COORD"])
            ...
            match_rows.append(
                {
                    ...
                    "TX_CROSSWAY_NL": crash_row.get("TX_CROSSWAY_NL"),
                    "CD_ROAD_TYPE_NL": crash_row.get("CD_ROAD_TYPE_NL"),
                    "TX_BUILD_UP_AREA_NL": crash_row.get("TX_BUILD_UP_AREA_NL"),
                    "TX_COLLISION_TYPE_NL": crash_row.get("TX_COLLISION_TYPE_NL"),
                    "TX_OBSTACLES_NL": crash_row.get("TX_OBSTACLES_NL"),
                }
            )
